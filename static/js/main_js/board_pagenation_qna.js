...
function createPcard(data) {
    fixcontainer.innerHTML = `
        <p class = "fix_p" id = "fix_p">${data.word} 에 대한<br>질문 요청 목록 입니다</p>
    `;
}
// 함수를 통해 Q&A 카드 생성
...
$.ajax({
    type: "POST",
    url: "http://3.34.3.84/api/question/list/", // 실제 URL로 변경해야 합니다.
    headers: {
    ...
    success: function (response) {
        ...
        createPaginationButtons(Math.ceil(data.questions.length / itemsPerPage), initialPageNumber); // 초기 페이지는 1로 설정
        createPcard(data)
        displayPageItems(initialPageNumber, data.questions);
