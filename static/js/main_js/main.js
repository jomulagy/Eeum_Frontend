document.addEventListener("DOMContentLoaded", function() {
    ...
    function createWordCard(data) {
        ...
        if (wordContainer.children.length < 8) {
            wordContainer.appendChild(wordItem);
        } else if (wordContainerRecent.children.length < 8) {
    ...
    // 가장 좋아요가 많은 8개 단어 보여주기
    wordsData.slice(0, 8).forEach(data => {
        createWordCard(data, wordContainer);
    });
    wordsData.sort((a, b) => a.index - b.index);
    // 인덱스 1번부터 8번까지의 단어 보여주기
    wordsData.slice(0, 8).forEach(data => {
        createWordCard(data);
    });
});