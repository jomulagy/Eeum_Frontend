...
function createPcard(data) {
    fixcontianer.innerHTML = `
        <p>${data.user} 님의<br>수정 요청 목록 입니다</p>
    `;
}
...
function createQnaCard(data) {
    ...
    qnaCard.addEventListener("click", function () {
        ...
        window.location.href = "/registrationrequest/list.html";
...
$.ajax({
    ...
    success: function (response) {
        // 서버로부터의 응답을 처리
        data = response;
        createPaginationButtons(Math.ceil(data.edits.length / itemsPerPage), initialPageNumber); // 초기 페이지는 1로 설정
        console.log(data.edits);
        createPcard(data)
        displayPageItems(initialPageNumber, data.edits);
